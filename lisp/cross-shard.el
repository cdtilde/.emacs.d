(defun send-query-in-region-to-all-shards (beg end)
  ""
  (interactive "r")
  (let* ((x (make-temp-file "foo"))
         (queryname (file-name-base x))
         (command (concat "~/Documents/Sears/Code/multi-select/query --shards 1-64 --sql " x " --name " queryname)))
    (message x)
    (message queryname)
    (write-region beg end x)
    (let ((command (concat "~/Documents/Sears/Code/multi-select/query --shards 1-64 --sql " x " --name " queryname)))
...
(defun sql-start-teradata ()
    (interactive)
    (sql-start-session "~/.sql-scripts/bteq" "db2"))


(defun sql-start-mysql-poc ()
    (interactive)
    (sql-start-session "~/.sql-scripts/mysql-poc" "mysql"))
...
(defun sql-start-local-mysql ()
    (interactive)
    (sql-start-session "~/.sql-scripts/mysql-local" "mysql"))
